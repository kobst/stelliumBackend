...
import { getCompletionShortOverview, getCompletionShortOverviewRelationships, getCompletionPlanets, getCompletionGptResponseChatThread, getCompletionShortOverviewForTopic, getCompletionForNatalPlanet, getCompletionForDominancePattern, expandPrompt, expandPromptRelationship, expandPromptRelationshipUserA, expandPromptRelationshipUserB, getCompletionGptResponseRelationshipChatThread } from '../services/gptService.js';
// import { processUserQueryAndAnswer } from '../services/vectorize.js';
import { generateRelevantNatalPositions, generateNatalPromptsShortOverview, generateDominanceDescriptions, getPlanetDescription, generateTopicMapping } from '../utilities/birthChartScoring.js';
import { BroadTopicsEnum } from '../utilities/constants.js';
import { retrieveTopicContext, upsertRecords, processTextSection, processTextSectionRelationship, processUserQueryForBirthChartAnalysis, processUserQueryForRelationshipAnalysis } from '../services/vectorize.js';
import { saveBasicAnalysis, saveTopicAnalysis, getBasicAnalysisByUserId, getUserSingle, getAllAnalysisByUserId, getTopicAnalysisByUserId, updateVectorizationStatus, updateRelationshipVectorizationStatus, fetchRelationshipAnalysisByCompositeId, getChatHistoryForBirthChartAnalysis, saveChatHistoryForBirthChartAnalysis, getChatHistoryForRelationshipAnalysis, saveChatHistoryForRelationshipAnalysis } from '../services/dbService.js';
        ...
        const planetPromises = planets.map(async (planet) => {
            ...
            try {
                ...
                const response = await Promise.race([
                    ...
                    new Promise((_, reject) => setTimeout(() => reject(new Error('GPT timeout')), 15000) // 15 second timeout
                    )
                ]);
...
export function writeToLog(logData, basicAnalysis = true) {
    ...
    try {
        ...
        // Write log file
        writeFileSync(join(logsDir, fileName), JSON.stringify(logData, null, 2));
        ...
        switch (section) {
            case 'overview': {
                if (birthChartBasicAnalysis.overview) {
                    const overviewRecords = await processTextSection(birthChartBasicAnalysis.overview, userId, "Overview");
...
export async function handleVectorizeTopicAnalysis(req, res) {
    ...
    try {
        ...
        if (topicData && topicData.subtopics[subtopic]) {
            ...
            const subtopicRecords = await processTextSection(topicData.subtopics[subtopic], userId, description);
...
export async function handleVectorizeRelationshipAnalysis(req, res) {
    ...
    try {
        ...
        if (relationshipAnalysis.analysis &&
            ...
            const categoryRecords = await processTextSectionRelationship(interpretationText, // Pass the interpretation text
            compositeChartId, description, category, relevantPositionData // Pass the relevant astrological positions
            );
...
export const handleProcessUserQueryForRelationshipAnalysis = async (req, res) => {
    ...
    try {
        ...
        const answer = await getCompletionGptResponseRelationshipChatThread(query, contextFromAnalysis, contextFromUserA, contextFromUserB, chatHistory, userAName, userBName);
