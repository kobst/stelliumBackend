...
import { getRawChartDataEphemeris, getRawChartDataEphemerisNoTime, findSynastryAspects, generateCompositeChart, scanTransitSeries, mergeTransitWindows, scanTransitToTransitAspects, mergeTransitToTransitWindows } from '../services/ephemerisDataService.js';
import { calculateTransitPriority, PLANET_SPEEDS } from '../utilities/transitPrioritization.js';
...
function convertWindowsToTransitEvents(windows, period, natalPlanets) {
    console.log("windows: ", windows[0]);
    ...
    return windows
        ...
        const transitingSigns = [];
        if (window.transitingSignAtStart)
            transitingSigns.push(window.transitingSignAtStart);
...
function transformDbTransitData(dbData) {
...
function convertTransitToTransitToEvents(windows, period) {
...
export async function handleGetTransitWindows(req, res) {
    try {
        ...
        const transitEvents = convertWindowsToTransitEvents(windows, { start: new Date(from), end: new Date(to) }, natalPlanets);
        ...
        // Convert transit-to-transit windows to events
        const transitToTransitEvents = convertTransitToTransitToEvents(mergedTransitWindows, { start: new Date(from), end: new Date(to) });
        res.json({
            transitEvents,
