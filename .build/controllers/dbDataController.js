// @ts-nocheck
// @ts-nocheck
import { saveCompositeChart, getCompositeCharts, saveSynastryChartInterpretation, getUsers, getUserSingle, saveRelationshipScoring, fetchRelationshipAnalysisByCompositeId, saveRelationshipAnalysis } from '../services/dbService.js';
...
export async function handleGenerateRelationshipAnalysis(req, res) {
    ...
    try {
        ...
        for (const categoryKey of Object.keys(RELATIONSHIP_CATEGORIES)) {
            ...
            const promptString = await generateRelationshipPrompt(userAName, userBName, categoryDisplayName, relationshipScores, formattedAstrology, contextA, contextB);
            console.log(`--- PROMPT FOR CATEGORY: ${categoryDisplayName} ---`);
            // console.log(promptString); 
            promises.push(getCompletionForRelationshipCategory(userAName, userBName, categoryDisplayName, relationshipScores, formattedAstrology, contextA, contextB)
                .then(interpretation => {
