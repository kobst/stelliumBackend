// @ts-nocheck
import { orbCodes, planetCodes, signCodes, relevantPromptAspects, relevantPromptAspectsV2, rulers, transitCodes, BroadTopicsEnum } from "./constants.js";
...
function findAspectsWithinOrb(planetName, birthData, orbAllowed = 5, k = null) {
    ...
    const aspectsSorted = birthData.aspects
        .filter(aspect => (aspect.aspectingPlanet.toLowerCase() === planetName.toLowerCase() ||
...
function findAspects(planetName, birthData, closeOnly = false) {
    ...
    let aspectsSorted = birthData.aspects
        .filter(aspect => (aspect.aspectingPlanet === planetName ||
...
export const generateRelevantNatalPositions = (promptKey, birthData, rulerMapping) => {
    ...
    let planets = prompt.planets.map(planet => {
        ...
        const matchingKey = Object.keys(rulerMapping).find(key => key.toLowerCase() === planet.toLowerCase());
...
function findAspectsMap(birthData, planetName) {
    const relevantAspects = birthData.aspects.filter(aspect => aspect.transitingPlanet === planetName || aspect.aspectingPlanet === planetName);
