// @ts-nocheck
import { planetCodes, signCodes, transitCodes, sortOrder } from "../constants.js";
...
export const findDailyTransitAspects = (dailyTransits) => {
    ...
    for (let i = 0; i < dailyTransits.length; i++) {
        ...
        for (let j = i + 1; j < dailyTransits.length; j++) {
            ...
            if (["South Node", "Chiron", "Part of Fortune"].includes(anotherPlanet.name))
                continue;
            // var transitAspects = []
    ...
    for (const key in groupedTransits) {
        ...
        if (groupedTransits.hasOwnProperty(key)) {
            ...
            for (let i = 1; i < group.length; i++) {
                const dateDiff = differenceInDays(parse(group[i].date, 'yyyy-MM-dd HH:mm:ss', new Date()), parse(group[i - 1].date, 'yyyy-MM-dd HH:mm:ss', new Date()));
                if (dateDiff > 1) {
...
function createSummaryTransit(key, group) {
    ...
    const closestOrbTransit = group.reduce((min, transit) => Math.abs(transit.orb) < Math.abs(min.orb) ? transit : min, group[0]);
