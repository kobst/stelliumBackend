// @ts-nocheck
import { elements, elementPoints, quadrants, modalities, ignorePlanets, ignorePointsForElements, ignorePoints } from "./constants.js";
...
function sortPlanetsByDegree(planets) {
    return planets
        ...
        if (degreeA === undefined && degreeB === undefined) {
            console.error('Missing degree for planets:', { planetA: a.name, planetB: b.name,
...
function isGrandTrine(planets) {
    ...
    function findConjunctions(mainPlanet, otherPlanets) {
        return otherPlanets.filter(planet => planet !== mainPlanet &&
...
function findYodPattern(planets) {
    ...
    function findConjunctions(apexPlanet, otherPlanets) {
        return otherPlanets.filter(planet => planet !== apexPlanet &&
...
function findStelliumByDegrees(planets) {
    ...
    let uniqueStelliums = stelliums.filter((stellium, _, array) => !array.some(other => other.length > stellium.length && other.includes(...stellium)));
