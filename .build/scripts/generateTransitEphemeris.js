...
export function generateSmartEphemeris(fromDateStr, toDateStr) {
    ...
    for (const date of allTimestamps) {
        const jd = sweph.julday(date.getUTCFullYear(), date.getUTCMonth() + 1, date.getUTCDate(), date.getUTCHours() + (date.getUTCMinutes() / 60) + (date.getUTCSeconds() / 3600), // More precise time
