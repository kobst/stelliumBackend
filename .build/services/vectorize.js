...
import { subTopicSearchPrompts } from '../utilities/constants.js';
...
export async function splitText(text, maxChunkSize = 900) {
    try {
...
export async function processUserQueryRelationship(compositeChartId, query) {
    ...
    try {
        ...
        console.log("Extracted data:", extractedData);
        // If you want to return a single string with all the information
        const combinedText = extractedData.map(data => `Relevant Aspects: ${data.relevantAspects}\nText: ${data.text}`).join('\n\n');
        console.log("Combined text:", combinedText);
        return combinedText;
    }
    catch (error) {
