import { planetCodes, signCodes, orbCodes, transitCodes, retroCodes, rulers } from '../constants.js';
...
export const decodePlanetHouseCode = (code) => {
    ...
    const planetCode = code.substring(1, 3);
    ...
    return description + ` (ref: ${origCode})`
}

export const decodePlanetCode = (code) => {
    const origCode = code
    // Extract components from the code
    // console.log(code + " code planet")
    const retroCode = code.substring(0, 1)
    const planetCode = code.substring(1, 3);
    // console.log(planetCode + " planet")
    const signCode = code.substring(3, 6);
    // console.log(signCode + " sign")
    const houseCode = code.substring(6);

    // console.log(houseCode + " house")


    // Find the matching descriptions
    const retro = getKeyByValue(retroCodes, retroCode)
    const planet = getKeyByValue(planetCodes, planetCode);
    const sign = getKeyByValue(signCodes, signCode);
    let house = ''
    if (houseCode) {
        house = parseInt(houseCode, 10); // Convert houseCode back to an integer
    }

    // Construct the description
    let description = `${retro} ${planet} in ${sign}`
    if (house !== '') {
        description = `${retro} ${planet} in ${sign} in the ${house} house`;
    }
    // if (isRetrograde) {
    //     description = `${planet} is retrograde in ${sign} in the ${house} house`;
    // }
...
export const decodeAspectCodeMap = (code) => {
    const origCode = code
    // console.log(code + " code full")
    code = code.substring(2); 
    // Extract the components from the code
    const planetA = code.substring(0, 8)
    const planetB = code.substring(12, 20)
    const planetCodeADescription = decodePlanetCode(planetA)
    const planetCodeBDescription = decodePlanetCode(planetB)

    const orbCode = code.substring(8, 9);
    // console.log(orbCode + "orbCode")
    const transitCode = code.substring(9, 12); // Includes "T"
    // console.log(transitCode + "transitCode")
    // const planetCodeBDescription = decodePlanetCode(7, 13)
    ...
    // Construct the description
    // const description = `${planetCodeADescription} ${orbDesc} ${aspectType} to ${planetCodeBDescription} (ref: ${origCode})`;
    const description = `${planetCodeADescription} ${orbDesc} ${aspectType} ${planetCodeBDescription} (ref: ${origCode})`;
    // console.log(description)
    return description;
}
...
export const decodeRulerCode = (code) => {
    ...
    const rulerPlanetCode = code.substring(1, 3);
    const signCode = code.substring(3, 6);
