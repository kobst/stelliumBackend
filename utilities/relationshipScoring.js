...
export function scoreRelationshipCompatibility(synastryAspects, compositeChart, userA, userB, compositeChartId, debug = true) {
    ...
    try {
        ...
        ALL_RELATIONSHIP_CATEGORIES.forEach(category => {
            ...
            const synastryResults = scoreSynastryCategoryAspects(category, synastryAspects, userA.firstName, userB.firstName);   
            // Add to raw scores
            ...
            if (debug) {
                debugLog.normalizedScores[category] = {
                    synastry: {
                        ...
                        // normalized: normalizedSynastry
                    },
                    composite: {
                        raw: rawScores[category].composite.score,
                        // normalized: normalizedComposite
                    },
                    synastryHousePlacements: {
                        raw: rawScores[category].synastryHousePlacements.score,
                        // normalized: normalizedsynastryHousePlacements
                    },
                    compositeHousePlacements: {
                        raw: rawScores[category].compositeHousePlacements.score,
                        // normalized: normalizedCompositeHousePlacements
                    },
                    overall: Math.round(overallScore)
                };
            }
 ...
 */
function scoreSynastryCategoryAspects(category, synastryAspects, userAFirstName, userBFirstName, debug = true) {
    const result = {
        score: 0,
                ...
                try {
                    ...
                    if (debug && result.debugInfo) {
                        ...
                        result.debugInfo.matchedAspects.push({
                            aspect: `${userAFirstName}'s ${aspect.planet1} in ${planet1Sign} ${aspect.aspectType} ${userBFirstName}'s ${aspect.planet2} in ${planet2Sign}`,
                            orb: aspect.orb,
                            score: score,
                            pairKey: aspectPair,
                ...
                if ((asp1Lower === p1Lower && asp2Lower === p2Lower) || 
                    ...
                    try {
                        ...
                        const score = calculateAspectScore(
                            aspect.aspectType, 
                            aspect.orb, 
                            p1, // Use the original pair key planets for consistency
                            p2, 
                            category
                        );
    ...
    try {
        return {
            ...
            planets: birthChart.planets ? birthChart.planets.map(p => {
                ...
                if (!p.name) console.log("WARNING: Planet missing name property");
                if (!p.sign) console.log("WARNING: Planet missing sign property");
                if (p.full_degree === undefined && p.degree === undefined) {
                    console.log("WARNING: Planet missing both full_degree and degree properties");
                }
                if (p.house === undefined) console.log("WARNING: Planet missing house property");
...
//     let normalized = (score / maxPossible) * 100;
