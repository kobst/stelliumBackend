    ...
    try {
        ...
        for (const logFile of logFiles) {
            try {
                ...
                const result = await processBirthChartAnalysisLog(
                    birthChartAnalysisLog.analysis, 
                    birthChartAnalysisLog.metadata.userId,
                    birthChartAnalysisLog.metadata.category
                );
        ...
        for (const [broadTopic, topicData] of Object.entries(BroadTopicsEnum)) {
            ...
            for (const [subtopicKey, subtopicLabel] of Object.entries(topicData.subtopics)) {
                ...
                try {
                    ...
                    const RAGResponse = await retrieveTopicContext(
                        userId,
                        subtopicLabel
                    );
